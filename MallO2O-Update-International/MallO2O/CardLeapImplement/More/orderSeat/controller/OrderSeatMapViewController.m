...
@interface OrderSeatMapViewController ()<MKMapViewDelegate>
{
...
- (NSArray *)generateAnnotations
{
    ...
    int i = 0;
    for (orderSeatInfo *info in self.shops) {
...
- (IBAction)gotoShop:(id)sender
{
    ...
    orderSeatInfo *info = self.shops[view.tag];
...
- (void)mapView:(MKMapView *)mapView didUpdateUserLocation:(MKUserLocation *)userLocation
{
    if (!self.myLocation) {
        ...
        if (latitude !=0 && longitude!=0) {
            [self getMyLocation];
            [self getShopList:baiDuLat lng:baiDuLng];
        }
    }
}
//地图中显示商家信息代理方法
...
- (void)mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated {
    if ([self.shops count]>0) {
        ...
        //当前位置没有获取到任何商家时候 移动便会走借口 确保附近商家出现
        baidu_lat = mapView.centerCoordinate.latitude;
        baidu_lng = mapView.centerCoordinate.longitude;
        [self getShopList:baidu_lat lng:baidu_lng];
    }
...
{
    NSString *tmp_url  =  [SwpTools swpToolGetInterfaceURL:@"seat_map"];
    ...
    [SwpRequest swpPOST:url parameters:dict isEncrypt:swpNetwork.swpNetworkEncrypt swpResultSuccess:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull resultObject) {
        if (swpNetwork.swpNetworkCodeSuccess == [resultObject[swpNetwork.swpNetworkCode] intValue]) {
            ...
            for (NSDictionary *dict in arr) {
                orderSeatInfo *info = [[orderSeatInfo alloc] initWithDictionary:dict];
