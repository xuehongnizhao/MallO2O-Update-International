//
//  ShopTakeOutSearchViewController.m
//  CardLeap
//
...
@interface ShopTakeOutSearchViewController ()<UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UISearchBar* searchBar;
@property (strong, nonatomic) UITableView *LinGroupTableView;

@end

@implementation ShopTakeOutSearchViewController
{
    //当前页数
...
-(void)initDataSourceAndOther
{
    ...
    [historyArray addObjectsFromArray:[[NSUserDefaults standardUserDefaults] objectForKey:@"takeout_history"]];
    //判断是否显示的历史记录
    isShowHistory=YES;
...
-(UITableView *)LinGroupTableView
{
    if (!_LinGroupTableView) {
        ...
        if ([_LinGroupTableView respondsToSelector:@selector(setSeparatorInset:)]) {
            [_LinGroupTableView setSeparatorInset:UIEdgeInsetsMake(0,0,0,0)];
        }
        
        if ([_LinGroupTableView respondsToSelector:@selector(setLayoutMargins:)]) {
            [_LinGroupTableView setLayoutMargins:UIEdgeInsetsMake(0,0,0,0)];
        }
    }
...
-(void)searchPost
{
    ...
    if (![_searchBar.text isEqualToString:@""])
    {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:historyArray forKey:@"takeout_history"];
...
-(void)initSearchBar
{
    _searchBar=[[UISearchBar alloc]initWithFrame:CGRectMake(38, 7, 220*Balance_Width, 30)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        static NSString *simpleTableIdentifier=@"search_take_out_cell";
        ...
        shopTakeoutInfo *info = [postDataSourceArray objectAtIndex:indexPath.row];
