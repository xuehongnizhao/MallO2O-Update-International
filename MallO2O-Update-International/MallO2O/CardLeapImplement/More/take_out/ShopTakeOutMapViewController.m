//
//  ShopTakeOutMapViewController.m
//  CardLeap
//
...
@interface ShopTakeOutMapViewController ()<MKMapViewDelegate>
{
...
@implementation ShopTakeOutMapViewController
...
- (void)showShopPane:(NSInteger)shopID
{
    ...
    shopTakeoutInfo *info = self.shops[shopID];
...
- (NSArray *)generateAnnotations
{
    ...
    int i = 0;
    for (shopTakeoutInfo *info in self.shops) {
...
- (IBAction)gotoShop:(id)sender
{
    ...
    shopTakeoutInfo *info = self.shops[view.tag];
    ShopTakeOutViewController *firVC = [[ShopTakeOutViewController alloc] init];
    [firVC setNavBarTitle:@"外卖详情" withFont:14.0f];
...
{
    NSString *tmp_url  =  [SwpTools swpToolGetInterfaceURL:@"shop_map"];
