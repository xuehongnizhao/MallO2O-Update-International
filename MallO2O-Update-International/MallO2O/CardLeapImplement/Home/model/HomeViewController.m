...
-(void)setIndexTableviewHeaderView
{
    ...
    // 社区服务 高度
    CGFloat faceViewHeight = 190*Balance_Heith;//这里按屏幕比率设置高度  以4.0屏幕为例
    CGFloat adbanderHeight = SCREEN_HEIGHT*26/100;
    CGFloat commenViewHeihgt = 120*Balance_Heith;
    //服务中心高度
    ...
    CGFloat industryViewHeight= 220*Balance_Heith;
    ...
    if ([cateArray count]>4) {
        view_height += 60*Balance_Heith;
    }else if ([cateArray count]<3){
        view_height -= 60*Balance_Heith;
    }else if ([cateArray count]==0){
        view_height -= 120*Balance_Heith;
    }
    ...
    if ([cateArray count]>4) {
        commenViewHeihgt += 60*Balance_Heith;
    }else if ([cateArray count]<3){
        commenViewHeihgt -= 60*Balance_Heith;
    }else if ([cateArray count]==0){
        commenViewHeihgt -= 120*Balance_Heith;
    }
...
-(void)setCateButton :(NSMutableArray*)array
{
    ...
    [cate autoSetDimension:ALDimensionHeight toSize:60.0f*Balance_Heith];
...
{
    if(!_industryView) {
        _industryView = [[IndustryZoneView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 220*Balance_Heith)];
