...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color
{
    ...
    if (self) {
        ...
        for (int i = 0; i < _numOfMenu; i++) {
            CGPoint position = CGPointMake( (i * 2 + 1) * textLayerInterval - 2, self.frame.size.height / 2 + 1);
            ...
            CAShapeLayer *indicator = [self creatIndicatorWithColor:_menuColor andPosition:CGPointMake(position.x + title.bounds.size.width / 2 + 10, self.frame.size.height / 2 + 1)];
            [self.layer addSublayer:indicator];
            [_indicators addObject:indicator];
            
            if (i != _numOfMenu - 1) {
                CGPoint separatorPosition = CGPointMake((i + 1) * separatorLineInterval + 5, self.frame.size.height / 2);
        ...
        _tableView.tintColor = UIColorFromRGB(DefaultColor);
        ...
        _secondTableview.tintColor = UIColorFromRGB(DefaultColor);
...
{
    if (tableView == _tableView) {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cellq"];
        if (!cell) {
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cellq"];
...
- (void)confiMenuWithSelectRow:(NSInteger)row isFirst:(BOOL)isFirst
{
    ...
    if (isFirst) {
        ...
        //        NSString *titleStr = [(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string];//当前选中的文本项
        //        NSDictionary *dic = _array[_currentSelectedMenudIndex];
        //        int currentRow;
        //        for (int i=0; i<[arr count]; i++) {
        //            NSString *tempTitle = [arr objectAtIndex:i];
        //            if (titleStr isEqualToString:tempTitle) {
        //                currentRow =
        //                break;
        //            }
        //        }
...
- (void) dismissMXPullDownMenu {
    ...
    //    [self animateBackGroundView:_backGroundView shonw:NO complete:^{
    //        [self animateTableView:_tableView show:NO complete:^{
    //
    //        }];
    //    }];
    ...
    NSLog(@"%li", (long)_currentSelectedMenudIndex);
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_tableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
