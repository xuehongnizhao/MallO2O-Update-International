...
- (void)viewDidLoad
{
    ...
    [self setNavBarTitle:@"城市列表" withFont:NAV_TITLE_FONT_SIZE];
...
//    NSLog(@"user_city:%@",USER_CITY);
    NSString *url = [NSString stringWithFormat:@"http://%@/action/ac_base/city_list", baseUrl];
    NSDictionary* dict=@{
                         @"app_key":url,
                         };
    ...
    [self swpPublicTooGetDataToServer:url parameters:dict isEncrypt:self.swpNetwork.swpNetworkEncrypt swpResultSuccess:^(id  _Nonnull resultObject) {
        NSArray * arr = [CityModule mj_objectArrayWithKeyValuesArray:[resultObject objectForKey:@"obj"]];
...
- (void)setCityInfo:(CityModule *)module{
    SetUserDefault(module.city_name, City_Name);
    SetUserDefault(module.c_id, City_ID);
...
-(void)clickButtonPopController
{
    ...
    SetUserDefault(selectCityName, City_Name);
    SetUserDefault(selectCityID, City_ID);
    [[NSUserDefaults standardUserDefaults] synchronize];
    [self.navigationController popViewControllerAnimated:YES];
}
...
- (HomeSelectedCityHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[HomeSelectedCityHeaderView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 50)];
