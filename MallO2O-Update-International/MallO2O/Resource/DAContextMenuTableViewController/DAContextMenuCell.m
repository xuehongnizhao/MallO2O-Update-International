...
@property (weak, nonatomic) IBOutlet UILabel *userAddressTitleView;
@property (weak, nonatomic) IBOutlet UILabel *userConsigneeView;
@property (weak, nonatomic) IBOutlet UILabel *userTelView;
...
- (void)setUp
{
    ...
    self.editable = YES;
    self.moreOptionsButtonTitle = NSLocalizedString(@"adressDetailCellEditTitle", nil);
    self.deleteButtonTitle      = NSLocalizedString(@"adressDetailCellDeleteTitle", nil);
    ...
    self.userAddressTitleView.text = NSLocalizedString(@"adressDetailCellAddressTitle", nil);
    self.userConsigneeView.text    = NSLocalizedString(@"adressDetailCellConsigneeTitle", nil);
    self.userTelView.text          = NSLocalizedString(@"adressDetailCellTelephoneTitle", nil);
    
    [self.userNameLabel sizeToFit];
    [self.userTelLabel sizeToFit];
    [self.addressLabel sizeToFit];
}

- (void)cellOfSelect:(BOOL)isSelect{
    if (isSelect) {
        _selectImageView.image = [UIImage imageNamed:@"address_sel"];
    }else{
        _selectImageView.image = [UIImage imageNamed:@"address_no"];
    }
}
...
//    CGFloat menuOptionButtonWidth = [self menuOptionButtonWidth];
    self.moreOptionsButton.frame = CGRectMake(width - 140, 0., 70, height);
    self.deleteButton.frame = CGRectMake(width - 70, 0., 70, height);
}

- (CGFloat)menuOptionButtonWidth
{
    NSString *string = [self.deleteButtonTitle isEqualToString:@"删除"]? self.deleteButtonTitle : self.moreOptionsButtonTitle;
//    CGFloat width = roundf([string sizeWithFont:self.moreOptionsButton.titleLabel.font].width + 2. * self.menuOptionButtonTitlePadding);
    CGFloat width = roundf([string sizeWithAttributes: @{NSFontAttributeName: [UIFont fontWithName:self.moreOptionsButton.titleLabel.font.familyName size:20]}].width + 2. * self.menuOptionButtonTitlePadding);
