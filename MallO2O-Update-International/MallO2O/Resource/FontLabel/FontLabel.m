...
- (void)drawTextInRect:(CGRect)rect {
	...
	if (self.zAttributedText == nil) {
		...
		if (self.numberOfLines == 1) {
			...
			if (self.adjustsFontSizeToFitWidth && self.minimumScaleFactor < actualFont.pointSize) {
				if (size.width > origSize.width) {
					...
					actualFont = [actualFont fontWithSize:MAX(MAX(desiredPointSize, self.minimumScaleFactor), 1.0f)];
