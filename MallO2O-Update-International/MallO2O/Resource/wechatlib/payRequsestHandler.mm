...
- ( NSMutableDictionary *)sendPay_demo
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = @"V3zhifuceshi";
    //订单金额,单位（分）
    NSString *order_price   = [WechatOrderInformation orderNumber].orderMoney;//1分钱测试
    NSLog(@"%@",[WechatOrderInformation orderNumber].orderNumber);
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给
    [packageParams setObject: NOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: [WechatOrderInformation orderNumber].orderId           forKey:@"out_trade_no"];//商户订单号
    ...
    if ( prePayid != nil) {
        ...
        package         = @"Sign=WXPay";
        //第二次签名参数列表
        NSLog(@"%@",[WechatOrderInformation orderNumber].orderId);
